...
/**
 * Trigger converting a lead to client (Module 2 stub)
 * POST /api/leads/:id/convert-to-client
 */
exports.convertToClientStub = async (req, res) => {
  try {
    const { id } = req.params;
    const currentUserId = req.user.id || req.user._id || req.user.userId;
    const isCompanyWide = isCompanyWideRole(req.user);
    ...
    if (lead.status === 'WON') {
      return sendError(res, 400, 'This lead has already been converted to WON status.');
    }

    if (lead.status === 'LOST') {
      return sendError(res, 400, 'A LOST lead cannot be directly converted to a Client without reactivating it first.');
    }
    ...
    return sendSuccess(res, 200, 'Lead successfully marked as WON and queued for Module 2 Client creation.', {
      leadId: lead._id,
      leadStatus: 'WON',
      module2Pending: true,
      message: 'Lead marked as WON. Full Client + ClientContact creation will execute via Module 2.'
    });
  } catch (error) {
    console.error('Error in convert-to-client stub:', error);
    return sendError(res, 500, error.message || 'Failed to trigger client conversion.');
  }
};

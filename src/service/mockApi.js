...
const purgeNirmanLocalStorageKeys = () => {
  try {
    const keysToRemove = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && (key.startsWith('nirman_') || key.startsWith('nirman-'))) {
...
export const getHRDashboardWidgets = async () => {
  await delay();
  const rawLogs = JSON.parse(localStorage.getItem('nirman_attendance_logs') || '[]');
  const logs = Array.isArray(rawLogs) ? rawLogs : [];
  const onlineCount = logs.filter(l => l && l.type === 'CLOCK_IN').length;

  const rawCorrections = JSON.parse(localStorage.getItem('nirman_corrections') || '[]');
  ...
  return {
    ...
    data: {
      totalUsers: 148,
      onlineCount,
      offlineCount: Math.max(0, 148 - onlineCount),
      pendingCorrections,
      securityAlerts: 0
    }
  };
